...
$dir_pos = index($NT_dir, "HTKNEWDEMO");
$get_dir = substr($NT_dir, $dir_pos, 11);


($get_dir =~ "HTKNEWDEMO") || die "Must be in directory HTKNEWDEMO to run this script\n";
...
if($configParams{"HCopy"} =~ /^[yY]/){
   &HCopy("mfc\\train", "lists\\trainfiles");
   &HCopy("mfc\\test", "lists\\testfiles");
}
